...
### 🎮 Parallels Status Toggle (`parallels_status_toggle/`)

DMM購入済み商品一覧の各商品カードに、Parallels上のWindows動作確認状態を手動でトグル記録するスクリプト。

**機能:**

- 各カードのタグコンテナ右端に1つの循環トグルボタンを配置
- クリックごとに 動作OK → 動作NG → 要Player → 未確認 → 動作OK… の順で切り替え
- 短縮ラベル（動作OK等）を表示し、ホバーで正式名称をツールチップ表示
- 設定を商品IDごとにlocalStorageへ永続化し、ページ再読み込み時に復元
- 無限スクロールで追加レンダリングされたカードにも自動でボタンを付与（MutationObserver）

**対象サイト:**

- `https://dlsoft.dmm.co.jp/library/*`

**動作:**

1. 各商品カードのタグコンテナ右端に循環トグルボタンを挿入
2. ボタンクリックで状態を切り替え、localStorageに保存
3. DOM監視で追加カードを検知し、保存済み状態を復元
...
3. **Parallels Status Toggle**: `parallels_status_toggle/wrapper.js`
